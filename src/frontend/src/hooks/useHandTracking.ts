    ...
    const initializeHandTracking = async () => {
      try {
                ...
                const handsData: HandLandmarks[] =
                  results.multiHandLandmarks.map(
                    (handLandmarks: any, idx: number) => ({
                      ...
                      handedness:
                        results.multiHandedness?.[idx]?.label || "Unknown",
                    }),
                  );

                setLandmarks(handsData);

                for (const handLandmarks of results.multiHandLandmarks as any[]) {
                  drawLandmarks(
                    ctx,
                    handLandmarks,
                    canvas.width,
        ...
        if (!cancelled && mountedRef.current) {
          setError(
            "Failed to initialize hand tracking. Check your internet connection.",
          );
          setIsTracking(false);
        }
      }
