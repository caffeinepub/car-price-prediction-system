...
function VolumeIcon({ volume }: { volume: number }) {
  if (volume === 0)
    return <VolumeX className="w-5 h-5 text-white flex-shrink-0" />;
  if (volume < 0.33)
    return <Volume className="w-5 h-5 text-white flex-shrink-0" />;
  if (volume < 0.66)
    return <Volume1 className="w-5 h-5 text-white flex-shrink-0" />;
