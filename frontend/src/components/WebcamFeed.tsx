...
import { Camera, CameraOff, Video, VideoOff, AlertCircle } from 'lucide-react';
...
export function WebcamFeed({ onActiveNotesChange, volume }: WebcamFeedProps) {
  ...
  const { playNote, stopNote, setVolume, isLoading: audioLoading, loadError } = useAudioPlayer();
  ...
  const { landmarks, isTracking, error: trackingError } = useHandTracking(videoRef, canvasRef, cameraActive);
  ...
  useEffect(() => {
    if (!landmarks || landmarks.length === 0) {
      ...
      previousNotesRef.current.forEach((note) => stopNote(note));
    ...
    previousNotesRef.current = activeNotesSet;
  ...
  if (cameraSupported === false) {
    return (
      <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20 text-center">
        ...
        <p className="text-white text-lg">Camera not supported in this browser</p>
      </div>
    );
  }
    ...
    <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-2xl">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {loadError && (
            <span className="text-xs text-orange-300 max-w-[200px] truncate" title={loadError}>
              🎵 Synth mode
            </span>
          )}
          {cameraActive && (
          ...
          <span className={`text-sm font-medium ${isTracking ? 'text-green-300' : 'text-white/50'}`}>
        ...
        {!cameraActive && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/70">
            <div className="text-center">
              ...
              <p className="text-white/70 text-sm">Click "Start Camera" to begin</p>
            </div>
          </div>
        )}
      ...
      <div className="mt-4 p-3 bg-white/5 rounded-lg border border-white/10">
        <p className="text-white/70 text-xs text-center">
          💡 Move your hand left/right across the frame to change notes. Extend 2+ fingers to play.
        </p>
      </div>
    </div>
  );
