...
export function PianoKeyboard({ activeNotes }: PianoKeyboardProps) {
  ...
  return (
    <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-2xl w-full">
      <h3 className="text-white text-xl font-semibold mb-4 text-center">Piano Keys (C4–C6)</h3>
      <div className="overflow-x-auto pb-2">
        <div className="relative mx-auto" style={{ width: `${totalWidth}px`, height: '220px' }}>
          {/* White keys */}
            ...
            {whiteKeys.map((key) => {
              ...
              return (
                <div
                  ...
                  style={{ width: `${whiteKeyWidth}px`, height: '200px', transformOrigin: 'top' }}
                >
          ...
          {/* Black keys */}
          <div className="absolute top-0 left-0 pointer-events-none" style={{ width: `${totalWidth}px` }}>
            {KEYS.map((key, index) => {
              if (!key.isBlack) return null;
              const isActive = activeNotes.includes(key.note);
              const whiteKeysBefore = KEYS.slice(0, index).filter((k) => !k.isBlack).length;
              const leftOffset = whiteKeysBefore * (whiteKeyWidth + whiteKeyGap) - 14;
