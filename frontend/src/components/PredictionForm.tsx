...
import {
  Loader2, Calendar, Gauge, Settings, Fuel, Users, X,
  Clock, ImagePlus, Trash2, Sparkles,
  CheckCircle, AlertCircle, TrendingDown, Car, ShieldCheck, Target
} from 'lucide-react';
...
export function PredictionForm({ onClose }: PredictionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const yearOfPurchaseNum = parseInt(yearOfPurchase);
    const usageDurationNum = parseInt(usageDuration);
    const ownersNum = parseInt(owners);

    if (isNaN(modelYearNum) || isNaN(mileageNum) || isNaN(yearOfPurchaseNum) || isNaN(usageDurationNum) || isNaN(ownersNum)) {
      toast.error('Please enter valid numbers for all numeric fields');
      return;
    }

    if (modelYearNum < 1886 || modelYearNum > BACKEND_CURRENT_YEAR) {
      toast.error(`Model year must be between 1886 and ${BACKEND_CURRENT_YEAR}`);
      return;
    }

    if (yearOfPurchaseNum < modelYearNum || yearOfPurchaseNum > BACKEND_CURRENT_YEAR) {
      toast.error(`Year of purchase must be between ${modelYearNum} and ${BACKEND_CURRENT_YEAR}`);
      return;
    }
    ...
    return (
      <section className="relative min-h-screen bg-background overflow-hidden">
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="orb orb-1 w-80 h-80 -top-20 -right-20 opacity-30" style={{ background: 'radial-gradient(circle, oklch(0.55 0.18 250 / 0.2), transparent 70%)' }} />
          <div className="orb orb-3 w-64 h-64 bottom-0 left-0 opacity-20" style={{ background: 'radial-gradient(circle, oklch(0.6 0.18 200 / 0.15), transparent 70%)' }} />
        </div>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="text-center mb-8 animate-bounce-in">
            ...
            <h2 className="text-3xl font-bold font-poppins gradient-text">Prediction Complete!</h2>
            <p className="text-muted-foreground text-sm mt-1">ML-powered multi-factor analysis</p>
          </div>
          ...
          <div className="flex justify-center mb-6 animate-card-enter stagger-1">
            <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl bg-gradient-to-r from-emerald-500/15 to-teal-500/15 border-2 border-emerald-500/40 shadow-lg">
              ...
              <div className="text-left">
                <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Model Accuracy</p>
                <p className="text-2xl font-black text-emerald-500 leading-none">{accuracyPct}%</p>
              </div>
          ...
          <div className="bg-card border border-border/50 rounded-3xl p-8 text-center hover-glow-card animate-card-enter stagger-2 mb-6">
            <p className="text-sm text-muted-foreground mb-2 font-medium">Estimated Current Value</p>
            <p className="text-5xl font-black gradient-text font-poppins">
              {formatINR(result.currentPrice.price)}
            </p>
            <p className="text-sm text-muted-foreground mt-2">
              Range: {formatINR(result.currentPrice.valueRange.low)} – {formatINR(result.currentPrice.valueRange.high)}
            </p>
          ...
          <div className="bg-card border border-border/50 rounded-2xl p-5 mb-6 animate-card-enter stagger-3">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Price Factors</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                { label: 'Brand Tier', value: formatINR(result.priceFactors.brandTierWeight) },
                { label: 'Fuel Premium', value: formatINR(result.priceFactors.fuelTypePremium) },
                { label: 'Transmission', value: formatINR(result.priceFactors.transmissionFactor) },
                { label: 'Mileage Impact', value: `-${formatINR(result.priceFactors.mileageImpact)}` },
                { label: 'Depreciation', value: `${(result.priceFactors.yearDepreciation * 100).toFixed(0)}%/yr` },
                { label: 'Recommendation', value: `${Math.min(99, Math.max(0, Math.round(result.recommendationScore * 10))) / 10}/10` },
              ].map(({ label, value }) => (
          ...
          {result.futurePredictions.length > 0 && (
            <div className="grid sm:grid-cols-3 gap-4 mb-6">
              {result.futurePredictions.map((pred, i) => (
                <div
                  key={i}
                  className={`bg-card border border-border/50 rounded-2xl p-5 text-center hover-glow-card animate-card-stagger-in stagger-${i + 4}`}
                >
                  <p className="text-xs text-muted-foreground mb-1 font-medium">
                    In {Number(pred.yearsAhead)} year{Number(pred.yearsAhead) > 1 ? 's' : ''}
                  </p>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={(e) => { handleDance(e); onClose(); }}
        ...
        <div className="flex items-center gap-2 mb-6 px-4 py-2.5 rounded-xl bg-emerald-500/8 border border-emerald-500/20 w-fit">
          ...
          <span className="text-sm font-medium text-emerald-600">Model accuracy: 86–99%</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 animate-card-enter">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="bg-card border border-border/50 rounded-2xl p-6 hover-glow-card transition-all duration-300">
              <Label htmlFor="modelYear" className="text-sm font-semibold mb-3 flex items-center gap-2">
            ...
            <div className="bg-card border border-border/50 rounded-2xl p-6 hover-glow-card transition-all duration-300">
              <Label htmlFor="mileage" className="text-sm font-semibold mb-3 flex items-center gap-2">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div className="bg-card border border-border/50 rounded-2xl p-6 hover-glow-card transition-all duration-300">
              <Label htmlFor="owners" className="text-sm font-semibold mb-3 flex items-center gap-2">
            ...
            <div className="bg-card border border-border/50 rounded-2xl p-6 hover-glow-card transition-all duration-300">
              <Label htmlFor="yearOfPurchase" className="text-sm font-semibold mb-3 flex items-center gap-2">
            ...
            <div className="bg-card border border-border/50 rounded-2xl p-6 hover-glow-card transition-all duration-300">
              <Label htmlFor="usageDuration" className="text-sm font-semibold mb-3 flex items-center gap-2">
                <Calendar className="w-4 h-4 text-primary" />
                Usage Duration (yrs) *
              </Label>
              <Input
          ...
          {predictMutation.isError && (
            <div className="flex items-center gap-3 p-4 rounded-xl bg-destructive/10 border border-destructive/20">
              ...
              <p className="text-sm text-destructive">
                {predictMutation.error?.message || 'Failed to predict car price. Please try again.'}
              </p>
            </div>
          )}
